    ...
    if faceCurFrame:
        for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
            ...
            if matches[matchIndex]:
                ...
                new_id = studentIDs[matchIndex]  # Assign the correct student ID

                if new_id != id:  # Only update studentInfo if it's a different person
                    id = new_id
                    if counter == 0:
            ...
            if counter == 1:
                try:
                    ...
                    if studentInfo:
                        ...
                        if len(array) > 0:  # Check if the image data is not empty
                            imgStudent = cv2.imdecode(array, cv2.COLOR_BGRA2BGR)

                            datetimeString = studentInfo.get('last_attendance_time')
                            if datetimeString:
                                ...
                                print(f"Student with ID {id} has no last_attendance_time.")
                        else:
                            print(f"Image data for student with ID {id} is empty.")
                    else:
                        print(f"Student with ID {id} not found.")
                except Exception as e:
                    print(f"An error occurred while processing student information: {str(e)}")
